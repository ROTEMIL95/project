...
export default function SentQuotes() {
            ...
            if (newStatus === 'approved' && oldStatus !== 'approved') {
                ...
                toast({
                 ...
                 toast({
                    ...
                    description: `הסטטוס של "${quote.projectName}" שונה ל: ${toHebrewStatus(newStatus)}`,
                });
            }
    ...
    const filteredQuotes = useMemo(() => {
        return quotes.filter(quote => {
            ...
            const matchesStatus = statusFilter === 'הכל' ||
                                  toHebrewStatus(quote.status || 'draft') === statusFilter;
    ...
    const prepareProfitByStatusData = () => {
        const statusProfit = {};
        sortedQuotes.forEach(quote => {
            const status = toHebrewStatus(quote.status || 'draft');
            // Simplified profit calculation: finalAmount - estimatedCost
            const totalProfit = (quote.finalAmount || 0) - (quote.estimatedCost || 0);
                                            ...
                                            <td className="px-3 py-3 text-center relative">
                                                ...
                                                <DropdownMenu>
                                                    <DropdownMenuTrigger asChild>
                                                        <Button variant="ghost" size="sm" className="h-6 px-2 text-xs">
                                                            <Badge className={cn("px-2 py-0.5 text-xs font-medium rounded-full cursor-pointer",
                                                                (quote.status || 'draft') === 'approved' && 'bg-green-100 text-green-800 hover:bg-green-200',
                                                                (quote.status || 'draft') === 'rejected' && 'bg-red-100 text-red-800 hover:bg-red-200',
                                                                (quote.status || 'draft') === 'cancelled' && 'bg-yellow-100 text-yellow-800 hover:bg-yellow-200',
                                                                (quote.status || 'draft') === 'sent' && 'bg-blue-100 text-blue-800 hover:bg-blue-200',
                                                                (quote.status || 'draft') === 'draft' && 'bg-gray-100 text-gray-800 hover:bg-gray-200'
                                                            )}>
                                                                {toHebrewStatus(quote.status || 'draft')}
                                                            </Badge>
                                                        </Button>
                                                    </DropdownMenuTrigger>
                                                    <DropdownMenuContent align="center">
                                                        ...
                                                        <DropdownMenuSeparator />
                                                        <DropdownMenuItem onClick={() => handleStatusChange(quote, 'approved')} className="text-green-700 hover:bg-green-50">אושר</DropdownMenuItem>
                                                        <DropdownMenuItem onClick={() => handleStatusChange(quote, 'rejected')} className="text-red-700 hover:bg-red-50">נדחה</DropdownMenuItem>
                                                        <DropdownMenuItem onClick={() => handleStatusChange(quote, 'cancelled')} className="text-yellow-700 hover:bg-yellow-50">בוטל</DropdownMenuItem>
                                                        <DropdownMenuItem onClick={() => handleStatusChange(quote, 'sent')} className="text-blue-700 hover:bg-blue-50">נשלח</DropdownMenuItem>
                                                        <DropdownMenuItem onClick={() => handleStatusChange(quote, 'draft')} className="text-gray-700 hover:bg-gray-50">טיוטה</DropdownMenuItem>
                                                    </DropdownMenuContent>
                                                </DropdownMenu>
                                            </td>
