...
export default function QuoteSummary({
    ...
    return (
        <div className="space-y-6">
            ...
            {/* NEW: Display separate paint/plaster room detail items */}
            {(() => {
                const paintRoomDetailItems = selectedItems.filter(
                    it => it.categoryId === 'cat_paint_plaster' && it.source === 'paint_room_detail'
                );

                if (paintRoomDetailItems.length === 0) return null;

                return (
                    <div className="mt-6 border rounded-lg p-4 bg-white" dir="rtl">
                        <div className="flex items-center justify-between mb-3">
                            <h3 className="text-lg font-semibold text-gray-800">פירוט חדרים — צבע ושפכטל</h3>
                        </div>

                        <div className="space-y-3">
                            {paintRoomDetailItems.map((item) => {
                                const hasWalls = Number(item.wallPaintQuantity || 0) > 0;
                                const hasCeiling = Number(item.ceilingPaintQuantity || 0) > 0;

                                return (
                                    <div key={item.id} className="rounded-md border p-3">
                                        {/* Room name */}
                                        <div className="text-sm font-medium text-gray-800 mb-2 flex justify-between items-center">
                                            <span>{item.roomName || item.name || 'חדר'}</span>
                                            <span className="font-bold text-gray-900">₪ {formatPrice(item.totalPrice || 0)}</span>
                                        </div>
                                            ...
                                            {hasWalls && (
                                                <div className="flex items-center justify-between rounded bg-gray-50 p-2">
                                                    <div className="text-gray-700">
                                                        <div className="font-semibold">קירות</div>
                                                        <div className="text-gray-600">
                                                            {item.wallPaintName && `סוג: ${item.wallPaintName} • `}
                                                            {item.wallPaintLayers > 0 && `שכבות: ${item.wallPaintLayers} • `}
                                                            כמות: {Number(item.wallPaintQuantity || 0).toFixed(0)} מ"ר
                                                        </div>
                                                    </div>
                                                </div>
                                            )}
                                            ...
                                            {hasCeiling && (
                                                <div className="flex items-center justify-between rounded bg-gray-50 p-2">
                                                    <div className="text-gray-700">
                                                        <div className="font-semibold">תקרה</div>
                                                        <div className="text-gray-600">
                                                            {item.ceilingPaintName && `סוג: ${item.ceilingPaintName} • `}
                                                            {item.ceilingPaintLayers > 0 && `שכבות: ${item.ceilingPaintLayers} • `}
                                                            כמות: {Number(item.ceilingPaintQuantity || 0).toFixed(0)} מ"ר
                                                        </div>
                                                    </div>
                                                </div>
                                            )}
